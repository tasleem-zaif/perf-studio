...
router.post('/config/trigger-token', async (req, res) => {
  ...
  try {
    const r = await apiRequest(
      ...
      'POST',
      { description: 'Peako trigger token' },
...
router.post('/generate-yaml', async (req, res) => {
  ...
  if (providers.includes('gitlab')) {
    ...
    const gitlabYaml = `# ============================================================
# Peako — GitLab CI/CD Performance Test Pipeline
...
`;
  ...
  if (providers.includes('github')) {
    ...
    const githubYaml = `# ============================================================
# Peako — GitHub Actions Performance Test Pipeline
# Generated by Peako on ${new Date().toISOString().slice(0, 19).replace('T', ' ')}
# ============================================================

name: Peako Performance Test
    ...
    steps:
      ...
      - name: Patch JMX parameters
        run: |
          ...
          python3 .Peako/patch_jmx.py "\$SCRIPT" "\${{ inputs.jmeter_users }}" "\${{ inputs.jmeter_rampup }}" "\${{ inputs.jmeter_loops }}" "\${{ inputs.jmeter_duration }}"
      ...
      - name: Cache Peako Docker image
        uses: actions/cache@v4
        with:
          path: /tmp/docker-cache
          key: docker-Peako-\${{ runner.os }}
          restore-keys: docker-Peako-

      - name: Load cached image or pull
        run: |
          if [ -f /tmp/docker-cache/Peako.tar ]; then
            echo "Loading Peako image from cache..."
            docker load -i /tmp/docker-cache/Peako.tar
          else
            echo "Pulling Peako image (first run on this runner)..."
            docker pull tasleemzaif/Peako:latest
            mkdir -p /tmp/docker-cache
            docker save tasleemzaif/Peako:latest -o /tmp/docker-cache/Peako.tar
          fi
...
`;
    ...
    const patcherPy = `# Peako JMX parameter patcher
...
`;
  ...
  try {
    ...
    if (gitCfg?.is_initialized && fs.existsSync(path.join(gitRoot, '.git'))) {
      ...
      // Configure identity
      await git.addConfig('user.name',  callerRow.name  || 'Peako');
      await git.addConfig('user.email', callerRow.email || 'noreply@Peako.com');
      ...
      if (status.staged.length > 0) {
        await git.commit('ci: add Peako Performance Test workflow [auto]');
...
router.post('/trigger', async (req, res) => {
  ...
  try {
    ...
    if (gitCfg?.is_initialized && fs.existsSync(path.join(wsRoot, '.git'))) {
      ...
      await git2.addConfig('user.name',  callerRow2?.name  || 'Peako');
      await git2.addConfig('user.email', callerRow2?.email || 'noreply@Peako.com');
    ...
    if (provider === 'github') {
      ...
      if (r.status === 404) {
        ...
        const wf = (wfList.body?.workflows || []).find(w =>
          w.path === `.github/workflows/${workflowFile}` || w.name === 'Peako Performance Test'
        );
        if (wf?.id) {
          r = await apiRequest(
      ...
      if (r.status === 204) {
        ...
        for (let attempt = 0; attempt < 5; attempt++) {
          ...
          const runsResp = await apiRequest(
            ...
            'GET', null,
            { Authorization: `token ${effectiveGithubToken}`, 'User-Agent': 'Peako', Accept: 'application/vnd.github+json' }
          );
    ...
    if (provider === 'bitbucket') {
      ...
      const bbBody = {
        target: {
          ...
          ref_name: bbRef,
          selector: { type: 'custom', pattern: 'Peako-test' },
        },
      ...
      const bbResp = await apiRequest(
        ...
        'POST', bbBody, { Authorization: `Basic ${bbAuth}`, 'User-Agent': 'Peako', Accept: 'application/json' }
      );
...
async function autoSyncCiRun(run, cfg, projectId, userId) {
  ...
  try {
    if (run.provider === 'github') {
      ...
      const ghHeaders = { Authorization: `token ${cfg.github_token}`, 'User-Agent': 'Peako', Accept: 'application/vnd.github+json' };
      ...
      await new Promise((resolve, reject) => {
        const fileStream = fs.createWriteStream(tmpZip);
        const options = { hostname: 'api.bitbucket.org', path: `/2.0/repositories/${cfg.bitbucket_workspace}/${cfg.bitbucket_repo_slug}/downloads/perf-results-${pipelineId2}.zip`, method: 'GET', headers: { Authorization: `Basic ${bbAuth2}`, 'User-Agent': 'Peako' }, rejectUnauthorized: false };
  ...
  try {
    ...
    if (run.provider === 'github') {
      const r = await apiRequest(
        ...
        'GET', null,
        { Authorization: `token ${cfg.github_token}`, 'User-Agent': 'Peako', Accept: 'application/vnd.github+json' }
      );
      if (r.status === 200) {
...
router.post('/runs/:runId/sync-results', async (req, res) => {
  ...
  try {
    ...
    if (run.provider === 'bitbucket') {
      ...
      await new Promise((resolve, reject) => {
        const fileStream = fs.createWriteStream(tmpZip);
        const options = { hostname: 'api.bitbucket.org', path: `/2.0/repositories/${cfg.bitbucket_workspace}/${cfg.bitbucket_repo_slug}/downloads/perf-results-${pipelineId3}.zip`, method: 'GET', headers: { Authorization: `Basic ${bbAuth3}`, 'User-Agent': 'Peako' }, rejectUnauthorized: false };
        https.request(options, response => {
          if (response.statusCode === 301 || response.statusCode === 302) {
            https.get(response.headers.location, { rejectUnauthorized: false }, r2 => {
              if (r2.statusCode !== 200 && r2.statusCode !== 206) {
