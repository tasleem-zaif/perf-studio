...
// Base URL the generated CI workflow/pipeline files use to reach POST /api/ci/scripts/decrypt —
// must be reachable from the customer's CI runner (GitHub/GitLab/Bitbucket hosted runners have
// normal internet egress, so this just needs to be Peako's own public backend URL).
const PEAKO_API_URL = (process.env.PEAKO_API_URL || `http://localhost:${process.env.PORT || 3001}`).replace(/\/$/, '');
...
function buildGithubWorkflowYaml({ dockerImage, k6Image, defaultScript, userBranch, scriptList, peakoApiUrl }) {
  return `# ============================================================
  ...
  workflow_dispatch:
    inputs:
      ...
      branch:
        ...
        default: '${userBranch}'
      license_token:
        description: 'Peako-issued run token — set automatically by Peako, do not fill in manually'
        required: false
        default: ''
...
jobs:
  jmeter:
    ...
    steps:
      ...
      # The checked-out .jmx/.js is encrypted ciphertext — Peako never pushes a runnable script
      # into a customer's repo. This step exchanges the run-scoped license_token (only ever
      # minted by Peako's own /trigger route, right after its license/VUH check passes) for the
      # real plaintext, written into a per-run temp folder outside the checkout. A manually
      # triggered run (no token) fails here, before any test runs.
      - name: Fetch decrypted test script
        run: |
          ...
          curl -sf -X POST "${peakoApiUrl}/api/ci/scripts/decrypt" \\
      ...
      # Backup cleanup — a trap set inside "Fetch decrypted test script" would only cover that
      # one step's own shell (each step here is a separate process), so the decrypted script's
      # temp folder is removed here instead, unconditionally, as the last thing this job does.
      - name: Clean up decrypted script
        if: always()
        run: rm -rf "\$TMP_DIR" 2>/dev/null || true
`;
}
...
run_jmeter:
  stage: test
  before_script:
    - apk add --no-cache curl jq${gitlabSshSetup}
    ...
    - |
      # The checked-out .jmx/.js is encrypted ciphertext — Peako never pushes a runnable script
      # into a customer's repo. This exchanges LICENSE_TOKEN (only ever set by Peako's own
      # /trigger route, right after its license/VUH check passes) for the real plaintext,
      # written into a per-job temp folder outside the checkout. A manually triggered pipeline
      # (no token) fails here, before any test runs.
      TMP_DIR="/tmp/peako-\$CI_JOB_ID"
      mkdir -p "\$TMP_DIR"
      chmod 700 "\$TMP_DIR"
      ...
      RESP_FILE="\$TMP_DIR/.resp.json"
      curl -sf -X POST "${PEAKO_API_URL}/api/ci/scripts/decrypt" \\
        -H "Content-Type: application/json" \\
        -d "{\\"token\\":\\"\$LICENSE_TOKEN\\"}" \\
        -o "\$RESP_FILE" || { echo "Peako license check failed — cannot decrypt script. Aborting before any test runs."; exit 1; }
      SCRIPT_FILENAME=\$(jq -r '.filename // empty' "\$RESP_FILE")
      if [ -z "\$SCRIPT_FILENAME" ]; then
        echo "Decrypt response missing filename — Peako may have rejected this run."
      ...
      rm -f "\$RESP_FILE"
      chmod 600 "\$SCRIPT_ABS"
      echo "TMP_DIR=\$TMP_DIR" >> peako.env
      echo "SCRIPT_ABS=\$SCRIPT_ABS" >> peako.env
  script:
    - . ./peako.env
    - mkdir -p reports
    - |
  ...
  artifacts:
    ...
    expire_in: 7 days
    when: always
  # Backup cleanup — after_script runs in its own context, separate from before_script/script,
  # so this recomputes the same deterministic temp path independently rather than relying on
  # the trap set above (which covers the normal case; this is the net for an abrupt job kill).
  after_script:
    - rm -rf "/tmp/peako-\$CI_JOB_ID" 2>/dev/null || true
  rules:
    - when: manual
`;
    try {
  ...
  if (providers.includes('github')) {
    ...
    const githubYaml = buildGithubWorkflowYaml({ dockerImage, k6Image, defaultScript, userBranch, scriptList, peakoApiUrl: PEAKO_API_URL });
    ...
    Peako-Performance-Test:
      ...
      - step:
          ...
          services:
            - docker
          script:
            - apk add --no-cache curl zip bash jq python3
            ...
            - |
              # The checked-out .jmx/.js is encrypted ciphertext — Peako never pushes a runnable
              # script into a customer's repo. This exchanges LICENSE_TOKEN (only ever set by
              # Peako's own /trigger route, right after its license/VUH check passes) for the
              # real plaintext, written into a per-run temp folder outside the checkout. A
              # manually triggered pipeline (no token) fails here, before any test runs.
              TMP_DIR="/tmp/peako-$PIPELINE_ID"
              mkdir -p "$TMP_DIR"
              chmod 700 "$TMP_DIR"
              trap 'rm -rf "$TMP_DIR"' EXIT INT TERM
              if [ -z "$LICENSE_TOKEN" ]; then
            ...
            - |
              ...
              if [ "$ENGINE" != "k6" ]; then
                ...
                python3 .PerfStudio/patch_jmx.py "$SCRIPT_ABS" "$JMETER_USERS" "$JMETER_RAMPUP" "$JMETER_LOOPS" "$JMETER_DURATION"
                echo "=== JMX state after patch ==="
                grep -E "num_threads|ramp_time|scheduler|duration|LoopController.loops|CSV_PATH|Argument.value" "$SCRIPT_ABS" | head -20 || true
              fi
            - |
              . ./peako.env
              SCRIPT_IN_CONTAINER="/decrypted/$(basename "$SCRIPT_ABS")"
...
router.post('/trigger', async (req, res) => {
  ...
  if (licenseOrgId) {
    ...
    // Short-lived, run-scoped token the CI job exchanges for the suite's plaintext script via
    // POST /api/ci/scripts/decrypt (routes/ciScripts.js) — the only thing standing between a
    // pushed-but-encrypted script and an actual test run. Only mintable here, right after the
    // license/VUH gate above already passed, and only if a real suite was matched — a manually
    // re-triggered workflow run (bypassing this route entirely) never gets one.
    if (matchedSuite2?.id) {
      ciScriptToken = jwt.sign(
        { orgId: licenseOrgId, projectId: Number(req.params.projectId), suiteId: matchedSuite2.id },
        CI_SCRIPT_TOKEN_SECRET,
        { expiresIn: '60m' }
      );
    }
  }
...
`;
      ...
      if (script_name) {
        ...
        // Place JMX at canonical path inside workspace — ENCRYPTED, since this workspace copy is
        // what actually gets committed and pushed to the customer's own git repo below (via
        // git2.add/commit/push or Bitbucket's Files API). srcAbs itself (Peako's own SSH-mode
        // working copy, backed up to S3 by generateScriptForSuite) is left untouched and plain —
        // auto-heal and future regeneration still read that copy directly.
        ...
        if (srcAbs && fs.existsSync(srcAbs)) {
          ...
          if (!fs.existsSync(canonicalDest)) {
            // Encrypt whenever we know the owning org (the normal case — every license-gated
            // trigger above resolves one); fall back to a plain copy only for the edge case of a
            // project whose owner has no org at all, so that unsupported state doesn't silently
            // block the script from reaching the workspace.
            if (licenseOrgId) {
              const plainContent = fs.readFileSync(srcAbs, 'utf8');
              const cipherContent = await encryptScript(plainContent, licenseOrgId);
              fs.writeFileSync(canonicalDest, cipherContent, 'utf8');
            } else {
              fs.copyFileSync(srcAbs, canonicalDest);
            }
          }
        }
      }
...
`.replace(/\r\n/g, '\n');
      ...
      //
      // Unlike SSH-mode, PAT-mode has no separate plaintext store outside this session — this
      // session IS what gets pushed below. So before encrypting what's about to be staged, back
      // the plaintext up to S3 (backupPatScriptPlaintext) first; readScriptContent() (used by
      // auto-heal and the CI decrypt endpoint) reads that backup instead of this session going
      // forward. The PSENC1: prefix check avoids re-encrypting (and re-backing-up bogus
      // "plaintext") content a previous trigger already encrypted here.
      let jmxContent = null;
      if (script_name) {
        const scriptFile = (script_name || '').replace(/\\/g, '/').split('/').pop();
        // Scoped by project only, not user — see the matching comment on matchedSuite2 above.
      ...
      if (_ghGitCfg?.is_initialized && _ghGitCfg?.remote_url) {
        ...
        if (!_ghSession.hadState) {
          ...
          const _githubYaml3 = buildGithubWorkflowYaml({
            ...
            userBranch: ghUserBranch, scriptList: _ghScriptList3, peakoApiUrl: PEAKO_API_URL,
          }).replace(/\r\n/g, '\n');
  ...
  try {
    ...
    if (provider === 'gitlab') {
      ...
      // Redeemed once by the pipeline's decrypt step (POST /api/ci/scripts/decrypt) to get the
      // suite's real plaintext content — the pushed .jmx/.js is encrypted ciphertext.
      params.append('variables[LICENSE_TOKEN]',   ciScriptToken || '');
    ...
    if (provider === 'github') {
      ...
      const dispatchBody = {
        ...
        inputs: {
          ...
          branch: targetRef,
          // Redeemed once by the workflow's decrypt step (POST /api/ci/scripts/decrypt) to get
          // the suite's real plaintext content — the pushed .jmx/.js is encrypted ciphertext.
          // Empty when no suite matched or the license/VUH gate above wasn't reached; the
          // workflow's decrypt step fails closed in that case, same as an expired token.
          license_token: ciScriptToken || '',
        },
      };
    ...
    if (provider === 'bitbucket') {
      ...
      const bbBody = {
        ...
        variables: [
          ...
          // Redeemed once by the pipeline's decrypt step (POST /api/ci/scripts/decrypt) to get
          // the suite's real plaintext content — the pushed .jmx/.js is encrypted ciphertext.
          { key: 'LICENSE_TOKEN', value: ciScriptToken || '', secured: true },
        ],
      };
