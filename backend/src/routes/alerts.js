...
router.post('/send-test', async (req, res) => {
  ...
  try {
    ...
    const info = await transport.sendMail({
      from:    `"${row.from_name||'Performance Studio'}" <${row.from_email}>`,
      to,
      replyTo: row.from_email,
      subject: `Performance Studio — SMTP Configuration Verified`,
      text: `Hello,\n\nYour Performance Studio SMTP configuration is working correctly.\nSent from: ${row.from_email}\n\nThis is a test email. If you received this, email alerts will be delivered to your inbox after each test run.\n\nThanks,\nPerformance Studio`,
      html: `<div style="font-family:Arial,sans-serif;max-width:520px;background:#1a2035;border-radius:12px;overflow:hidden;">
        ...
        <div style="padding:24px;">
          ...
          <p style="color:#b8c4d8;margin:0 0 8px;font-size:13px;">Your Performance Studio email alerts are configured correctly.</p>
        ...
        <div style="background:#0f172a;padding:10px 24px;border-top:1px solid #2e3a55;text-align:center;">
          <p style="color:#4b5563;font-size:11px;margin:0;">Performance Studio — Automated Test Reporting</p>
        </div>
      </div>`,
      headers: {
