...
function getInviteTransport(userId) {
  ...
  if (!isUsable(cfg)) {
    ...
    if (superAdmin && superAdmin.id !== userId) {
      const superCfg = db.prepare('SELECT * FROM alert_configs WHERE user_id = ?').get(superAdmin.id);
...
router.post('/', auth, async (req, res) => {
  const inviter = db.prepare('SELECT * FROM users WHERE id = ?').get(req.userId);
  ...
  if (inviter.role === 'super_admin' && role === 'org_admin' && !req.body.org_id)
    return res.status(400).json({ error: 'You must select an organization for the Org Admin. Create one first under Organizations tab.' });
  ...
  db.prepare("UPDATE invites SET status='expired' WHERE email=?").run(email);
  ...
  ).run(email, name || '', role, orgId, req.userId, token, expiresAt);

  // Build accept URL — prefer URL sent by browser (works for ngrok, LAN IP, production)
  const frontendUrl = (req.body.frontend_url && req.body.frontend_url.startsWith('http'))
  ...
  const email_ = buildInviteEmail({ role }, acceptUrl, inviter.name, org?.name || 'PerfStudio');
  let emailSent = false;

  // Send invite email whenever SMTP is configured — don't require enabled=true
  // (the enabled flag is for automated test-run alerts, not explicit invite actions)
  if (transport) {
    try {
...
router.get('/', auth, (req, res) => {
  const inviter = db.prepare('SELECT role, org_id FROM users WHERE id = ?').get(req.userId);
  let invites;
...
router.delete('/:id', auth, (req, res) => {
  db.prepare("UPDATE invites SET status = 'expired' WHERE id = ?").run(req.params.id);
  res.json({ ok: true });
});
...
router.get('/validate/:token', (req, res) => {
  ...
  const org     = invite.org_id ? db.prepare('SELECT name FROM organizations WHERE id = ?').get(invite.org_id) : null;
...
router.get('/org-users', auth, (req, res) => {
  ...
  const users = db.prepare(`
    SELECT u.id, u.email, u.name, u.role, u.status,
      GROUP_CONCAT(pa.project_id) as assigned_project_ids
    FROM users u
...
router.put('/assign/:userId', auth, (req, res) => {
  const caller = db.prepare('SELECT role, org_id FROM users WHERE id = ?').get(req.userId);
  if (!['super_admin', 'org_admin'].includes(caller.role))
    return res.status(403).json({ error: 'Forbidden' });
