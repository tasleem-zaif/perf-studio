...
async function getGlobal(userId) {
  const row = await db.prepare('SELECT config_json FROM global_config WHERE user_id = ?').get(userId);
  return row ? JSON.parse(row.config_json) : {};
}

async function getProject(projectId) {
  const row = await db.prepare('SELECT config_json FROM project_config WHERE project_id = ?').get(projectId);
  return row ? JSON.parse(row.config_json) : {};
}
...
router.get('/', async (req, res) => {
  ...
  const global = await getGlobal(req.userId);
...
router.put('/', async (req, res) => {
  ...
  const existing = await db.prepare('SELECT id FROM project_config WHERE project_id = ?').get(req.params.projectId);
  if (existing) {
    await db.prepare('UPDATE project_config SET config_json = ? WHERE project_id = ?')
      .run(JSON.stringify(cfg), req.params.projectId);
  } else {
    await db.prepare('INSERT INTO project_config (project_id, config_json) VALUES (?, ?)')
