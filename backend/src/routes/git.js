...
async function ensureUserWorkspace(gitRoot, cfg, user) {
  ...
  const authorEmail = identity?.author_email || user.email || cfg.email || 'noreply@Peako.com';
...
router.post('/init', async (req, res) => {
  ...
  try {
    ...
    await git.addConfig('user.email', cfg.email || caller.email || 'noreply@Peako.com');

    // Create .gitignore at workspace root
    const gitignore = path.join(gitRoot, '.gitignore');
    fs.writeFileSync(gitignore, [
      '# Peako — ignore large binary files and temp artifacts',
      '*_workspace/',
      '*.log',
      '*.tmp',
      'node_modules/',
...
Performance test project managed by **Peako** — AI-Powered Performance Testing.
...
`);
    ...
    if (status.staged.length > 0 || status.not_added.length > 0 || status.modified.length > 0) {
      await git.add('.');
      await git.commit('Initialize: Peako project structure');
    }
    ...
    if (remoteRefs.includes(`refs/heads/${baseBranch}`)) {
      ...
      try {
        ...
        await git.add('.');
        await git.commit('Initialize: Peako project structure (resolved merge)');
      }
      await disableGcm(git);
    ...
    db.prepare('INSERT INTO git_commits (project_id,user_id,branch,message,pushed) VALUES (?,?,?,?,?)')
      .run(req.params.projectId, req.userId, baseBranch, 'Initial commit: Peako project structure', 1);
...
router.post('/push', async (req, res) => {
  ...
  try {
    ...
    await git.addConfig('user.email', userIdentity?.author_email || caller.email || cfg.email || 'noreply@Peako.com');
...
// ── PUT /prs/:prId/mark-merged — admin marks PR merged (when merged outside Peako) ──
