...
const { resolveOrgSlugForProject, resolveWorkspaceRoot } = require('../utils/projectFolders');
...
async function scaffoldWorkspaceInSession(session, proj, projectId) {
...
`);
  ...
  const collections = await db.prepare('SELECT * FROM collections WHERE project_id = ?').all(projectId);
...
router.post('/init', async (req, res) => {
  ...
  try {
    ...
    if (!isSSH) {
      ...
      await scaffoldWorkspaceInSession(session, proj, req.params.projectId);
  ...
  try {
    ...
    if (!isSSH) {
      ...
      if (branchExistsOnRemote) {
        ...
        await gitEngine.persistSession(session, gitDir, orgSlug);
        return res.json({ ok: true, message: `Pulled latest from origin/${branch}.` });
      } else {
...
router.post('/sync', async (req, res) => {
  ...
  try {
    ...
    if (!isSSH) {
      ...
      await gitEngine.persistSession(session, gitDir, orgSlug);
      return res.json({ ok: true, message: `Branch "${branch}" synced with latest ${baseBranch}.` });
    }
