...
router.get('/', (req, res) => {
  const row = db.prepare('SELECT config_json FROM global_config WHERE user_id = ?').get(req.userId);
...
router.put('/', (req, res) => {
  ...
  const existing = db.prepare('SELECT id FROM global_config WHERE user_id = ?').get(req.userId);
  if (existing) {
    db.prepare('UPDATE global_config SET config_json = ? WHERE user_id = ?')
      .run(JSON.stringify(cfg), req.userId);
  } else {
    db.prepare('INSERT INTO global_config (user_id, config_json) VALUES (?, ?)')
      .run(req.userId, JSON.stringify(cfg));
  }
  // Refresh config.json in all collection/env folders (global config is included there)
  setImmediate(() => {
    const projects = db.prepare('SELECT id FROM projects WHERE user_id = ?').all(req.userId);
